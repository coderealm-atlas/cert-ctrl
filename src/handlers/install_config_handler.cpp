...
namespace certctrl
{
  ...
  monad::IO<void> InstallConfigHandler::handle_clear_cache()
  {
    ...
    std::cerr << "Cleared cached install-config state (memory only)." << std::endl;
    return monad::IO<void>::pure();
  }
      ...
      const PullOptions &options)
  {
    ...
    return run_copy_stage()
        .then([this]() { return run_import_stage(); })
        .then([this]() {
          output_.logger().info()
        ...
        .catch_then([this](monad::Error err) -> ReturnIO {
  ...
  monad::IO<void> InstallConfigHandler::run_copy_stage()
  {
    ...
    if (options.cert_id && options.ca_id)
    {
      return install_config_manager_->apply_copy_actions(
          ...
          .then([this]() {
            const auto &config_inner = *active_config_;
            const auto &options_inner = *active_options_;
            return install_config_manager_->apply_copy_actions(
  ...
  monad::IO<void> InstallConfigHandler::run_import_stage()
  {
    ...
    BOOST_LOG_SEV(app_logger(), trivial::trace)
        ...
        << " target_id="
        << (target_id ? std::to_string(*target_id) : "<none>")
        << " skip_import=" << (options.skip_import ? "true" : "false");
    DEBUG_PRINT(
        "InstallConfigHandler::apply_copy_and_import - calling apply_import_ca_actions");
