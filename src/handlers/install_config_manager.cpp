...
bool InstallConfigManager::is_private_material_name(const std::string &name) {
  return name == "private.key" || name == "bundle.pfx" ||
         name == "certificate.pfx";
}

std::filesystem::perms
InstallConfigManager::desired_permissions(bool private_material) {
#ifdef _WIN32
  return std::filesystem::perms::owner_all;
#else
  if (private_material) {
...
std::optional<std::string> InstallConfigManager::perform_copy_operation(
  ...
  try {
    for (const auto &item : config.installs) {
      ...
      if (!item.from || item.from->empty()) {
        return ReturnIO::fail(Error{.code = my_errors::GENERAL::INVALID_ARGUMENT,
      ...
      if (auto ensure_err = ensure_resource_materialized_sync(item);
      ...
      if (*item.ob_type == "cert") {
        ...
        return ReturnIO::fail(Error{.code = my_errors::GENERAL::INVALID_ARGUMENT,
                                    .what = "Unsupported ob_type for copy"});
      }

      resource_root /= std::to_string(*item.ob_id);
      resource_root /= "current";
      ...
      for (std::size_t i = 0; i < item.from->size(); ++i) {
        ...
        if (auto err = perform_copy_operation(source_path, dest_path,
                                              private_material)) {
          return ReturnIO::fail(Error{.code = my_errors::GENERAL::FILE_READ_WRITE,
                                      .what = *err});
        }
    ...
    return ReturnIO::fail(Error{.code = my_errors::GENERAL::UNEXPECTED_RESULT,
                                .what = e.what()});
  }
}
