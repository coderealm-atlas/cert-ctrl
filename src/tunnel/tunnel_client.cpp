...
EndpointParts ParseEndpoint(const std::string &endpoint) {
	...
	if (!parsed) {
		throw std::runtime_error(
				fmt::format("invalid tunnel endpoint '{}': {}", endpoint,
...
LocalEndpointParts ParseLocalEndpoint(const std::string &endpoint) {
	...
	if (!parsed) {
		throw std::runtime_error(
				fmt::format("invalid local_base_url '{}': {}", endpoint,
										parsed.error().message()));
	}
	const auto &url = parsed.value();
	if (!url.has_authority() || url.host().empty()) {
	...
	if (url.scheme() != "http") {
		throw std::runtime_error(
				fmt::format("local_base_url must use http:// scheme (got '{}')",
										url.scheme()));
	}
...
public:
	Session(TunnelClient &client, TunnelConfig config, EndpointParts endpoint,
			...
			: client_(client), config_(std::move(config)),
				...
				max_payload_bytes_(static_cast<std::size_t>(
						std::max(0, config_.max_payload_bytes))),
				resolver_(net::make_strand(client.ioc_)),
				ssl_ctx_(ssl::context::tls_client),
				ws_(net::make_strand(client.ioc_), ssl_ctx_),
				ping_timer_(client.ioc_) {
		ConfigureSsl();
		ws_.text(true);
	}
...
private:
	...
	private:
		void BuildHttpRequest(Session &session) {
			...
			const std::string host_value =
					local_endpoint_.host_header.empty() ? local_endpoint_.host
		...
		void OnResolve(const beast::error_code &ec,
			...
			stream_.async_connect(results, beast::bind_front_handler(
																&LocalCall::OnConnect, shared_from_this()));
		}
		...
		void OnConnect(const beast::error_code &ec,
			...
			http::async_write(stream_, http_request_,
												beast::bind_front_handler(&LocalCall::OnWrite,
																								 shared_from_this()));
		}
		...
		void OnWrite(const beast::error_code &ec, std::size_t) {
			...
			http::async_read(stream_, buffer_, http_response_,
											 beast::bind_front_handler(&LocalCall::OnRead,
																								shared_from_this()));
		}
	...
	void Resolve() {
		client_.output_.logger().info()
				<< "Tunnel resolving " << endpoint_.host << ':' << endpoint_.port
				<< std::endl;
		resolver_.async_resolve(
	...
	void OnResolve(const beast::error_code &ec,
		...
		beast::get_lowest_layer(ws_)
				.async_connect(results, beast::bind_front_handler(&Session::OnConnect,
																													shared_from_this()));
	}
		...
		ws_.set_option(websocket::stream_base::decorator(
				[](websocket::request_type &req) {
					req.set(http::field::user_agent,
									std::string("cert-ctrl-tunnel/") +
											BOOST_BEAST_VERSION_STRING);
				}));
		ws_.async_handshake(endpoint_.host, endpoint_.target,
												beast::bind_front_handler(&Session::OnWsHandshake,
																								 shared_from_this()));
	}
	...
	void HandleRequest(const json::value &jv) {
		...
		if (config_.max_concurrent_requests > 0 &&
			...
			SendImmediateError(req.id, 429,
												 "too many concurrent tunnel requests");
			return;
		}
...
void TunnelClient::Start() {
	...
	output_.logger().info()
			<< "Tunnel client (preview) enabling for " << config.remote_endpoint
...
void TunnelClient::LogConfiguration(const TunnelConfig &config) {
	output_.logger().debug()
			<< fmt::format(
						 "tunnel cfg: ping={}s, timeout={}s, max_concurrent={}, max_payload={}B",
...
void TunnelClient::StartSession(TunnelConfig config) {
	...
	try {
		...
		output_.logger().error() << "Tunnel configuration error: " << ex.what()
														 << std::endl;
		ScheduleReconnect();
		return;
	}
