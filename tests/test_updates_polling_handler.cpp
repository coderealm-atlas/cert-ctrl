...
struct TestEnvFixture : public ::testing::Test {
  ...
  void SetUp() override {
    ...
    std::error_code ec; std::filesystem::create_directories(tmp_root_, ec);
    auto write_json = [](const std::filesystem::path &p, const json::object &o){ std::ofstream ofs(p); ofs << json::serialize(o); };
    // minimal httpclient config so provider succeeds
    write_json(tmp_root_ / "httpclient_config.json", json::object{{"threads_num",1},{"ssl_method","tlsv12_client"},{"insecure_skip_verify", true},{"verify_paths", json::array{}},{"certificates", json::array{}},{"certificate_files", json::array{}},{"proxy_pool", json::array{}}});
    // application.json for base url (use default helper)
    write_json(tmp_root_ / "application.json", json::object{{"auto_fetch_config", false},{"verbose","info"},{"url_base", testutil::url_base()}});
    // ioc_config.json minimal
    write_json(tmp_root_ / "ioc_config.json", json::object{{"threads_num",1},{"name","updates-test-env"}});
    config_paths_.assign({tmp_root_});
    profiles_.clear();
    config_sources_ptr_ = std::make_unique<cjj365::ConfigSources>(config_paths_, profiles_);
  certctrl::CliParams params; params.subcmd = "updates"; params.config_dirs = {tmp_root_}; params.keep_running = false;
  cli_ctx_ptr_ = std::make_unique<certctrl::CliCtx>(po::variables_map{}, std::vector<std::string>{"updates"}, std::vector<std::string>{}, std::move(params));
    clictx_ = cli_ctx_ptr_.get();
    // Simple stub provider to avoid file load race (we already write file but fixture only needs minimal values)
    struct HttpClientConfigProviderTest : cjj365::IHttpclientConfigProvider {
      cjj365::HttpclientConfig cfg; // default values: tlsv12_client, threads_num=0 -> auto, insecure=false
      const cjj365::HttpclientConfig &get() const override { return cfg; }
    };
    auto injector = di::make_injector(
        di::bind<cjj365::ConfigSources>().to(*config_sources_ptr_),
        di::bind<cjj365::IHttpclientConfigProvider>().to<HttpClientConfigProviderTest>(),
        di::bind<cjj365::IIocConfigProvider>().to<cjj365::IocConfigProviderFile>(),
        di::bind<cjj365::IIoContextManager>().to<cjj365::IoContextManager>().in(di::singleton),
        di::bind<customio::IOutput>().to(testinfra::shared_output()),
        di::bind<certctrl::CliCtx>().to(*cli_ctx_ptr_),
        di::bind<certctrl::ICertctrlConfigProvider>().to<certctrl::CertctrlConfigProviderFile>().in(di::singleton));
...
protected:
  ...
  void SetUp() override {
    ...
    std::error_code ec; std::filesystem::create_directories(tmp_root_, ec);
    auto write_json = [](const std::filesystem::path &p, const json::object &o){ std::ofstream ofs(p); ofs << json::serialize(o); };
    write_json(tmp_root_ / "application.json", json::object{{"auto_fetch_config", false},{"verbose","info"},{"url_base", base_url_}});
    write_json(tmp_root_ / "httpclient_config.json", json::object{{"threads_num",1},{"ssl_method","tlsv12_client"},{"insecure_skip_verify", true},{"verify_paths", json::array{}},{"certificates", json::array{}},{"certificate_files", json::array{}},{"proxy_pool", json::array{}}});
    write_json(tmp_root_ / "ioc_config.json", json::object{{"threads_num",1},{"name","updates-real"}});
    config_paths_.clear(); config_paths_.push_back(tmp_root_);
    profiles_.clear();
    config_sources_ptr_ = std::make_unique<cjj365::ConfigSources>(config_paths_, profiles_);
    params_ = certctrl::CliParams{}; params_.subcmd = "updates"; params_.config_dirs = {tmp_root_};
    positional_.assign({"updates"});
    unrecognized_.assign({"updates"});
    cli_ctx_ptr_ = std::make_unique<certctrl::CliCtx>(po::variables_map{}, std::vector<std::string>(positional_), std::vector<std::string>(unrecognized_), certctrl::CliParams(params_));
...
TEST_F(UpdatesRealServerFixture, DeviceRegisterThenPollUpdates) {
  ...
  std::string email = std::getenv("TEST_EMAIL") ? std::getenv("TEST_EMAIL") : testutil::LOGIN_EMAIL;
  std::string password = std::getenv("TEST_PASSWORD") ? std::getenv("TEST_PASSWORD") : testutil::LOGIN_PASSWORD;
  testutil::login_io(*http_mgr_, base_url_, email, password).run([&](auto r){ login_r = std::move(r); login_notifier.notify(); });
  login_notifier.waitForNotification();
  if (!login_r || login_r->is_err()) {
    GTEST_SKIP() << "login failed: " << (!login_r ? "no result" : login_r->error().what);
  }
  auto session_cookie = login_r->value().session_cookie;
  if (session_cookie.empty()) GTEST_SKIP() << "missing session cookie";
  // device_start
  misc::ThreadNotifier start_notifier(5000);
  std::optional<monad::Result<testutil::DeviceStartData, monad::Error>> start_r;
  testutil::device_start_io(*http_mgr_, base_url_, session_cookie).run([&](auto r){ start_r = std::move(r); start_notifier.notify(); });
  start_notifier.waitForNotification();
  if (!start_r || start_r->is_err()) {
    GTEST_SKIP() << "device_start failed: " << (!start_r ? "no result" : start_r->error().what);
  }
  auto device_code = start_r->value().device_code;
  if (device_code.empty()) GTEST_SKIP() << "missing device_code";
  std::string access_token; std::string refresh_token; std::string last_status;
  for (int attempt=0; attempt<5; ++attempt) {
    misc::ThreadNotifier poll_notifier(5000);
    std::optional<monad::Result<testutil::DevicePollData, monad::Error>> poll_r;
    testutil::device_poll_io(*http_mgr_, base_url_, device_code).run([&](auto r){ poll_r = std::move(r); poll_notifier.notify(); });
    poll_notifier.waitForNotification();
    if (!poll_r || poll_r->is_err()) { last_status = poll_r && poll_r->is_err() ? poll_r->error().what : "error"; break; }
    last_status = poll_r->value().status;
    if (!poll_r->value().access_token.empty()) { access_token = poll_r->value().access_token; refresh_token = poll_r->value().refresh_token; break; }
    std::this_thread::sleep_for(1500ms);
  }
  if (access_token.empty()) {
    GTEST_SKIP() << "device_poll did not yield access token; last status=" << last_status;
  }
  auto write_text = [](const std::filesystem::path &p, const std::string &s){ std::ofstream ofs(p, std::ios::trunc); ofs << s; };
  write_text(tmp_root_ / "access_token.txt", access_token);
  if (!refresh_token.empty()) write_text(tmp_root_ / "refresh_token.txt", refresh_token);
  ...
  handler_->start().run([&](auto r){ updates_r = std::move(r); updates_notifier.notify(); });
  updates_notifier.waitForNotification();
  if (!updates_r) { GTEST_SKIP() << "no result from updates handler"; }
  if (updates_r->is_err()) {
    if (updates_r->error().code == 401 || updates_r->error().code == 403) {
      FAIL() << "updates polling auth failure: " << updates_r->error().what;
    } else {
      GTEST_SKIP() << "updates handler error: code=" << updates_r->error().code << " msg=" << updates_r->error().what;
    }
  }
  ...
  if (status == 200) {
    ...
    for (auto &sig : resp.data.signals) {
      if (data::is_install_updated(sig)) ++counted_install;
      else if (data::is_cert_renewed(sig)) ++counted_renewed;
      else if (data::is_cert_revoked(sig)) ++counted_revoked;
    }
  ...
  EXPECT_TRUE(handler_->parse_error().empty()) << "parse error: " << handler_->parse_error();
  SUCCEED();
}
