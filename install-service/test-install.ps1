`# cert-ctrl installation script (PowerShell)
# Generated by: https://install.example
# Mirror: primary
# Version: v0.0.0
...
$paramForceInstall = ([bool]$Force) -or ("false" -eq "true")
...
elseif (-not $paramUserInstall -and -not (Test-Administrator)) {
    ...
    Write-Info "Option 2: Run './install.ps1 -UserInstall' to install for only the current user at '$userInstallPath'."
    Write-Info "You can also specify -InstallDir to point at a directory you already own."
    exit 1
}

$mirrorUrl = "https://install.example/releases/proxy"
if ($mirrorUrl -eq "https://install.example/releases/proxy") {
    $packageUrl = "$mirrorUrl/$Version/cert-ctrl-windows-$archSlug.zip"
} else {
    $packageUrl = "$mirrorUrl/coderealm-atlas/cert-ctrl/releases/download/$Version/cert-ctrl-windows-$archSlug.zip"
}
