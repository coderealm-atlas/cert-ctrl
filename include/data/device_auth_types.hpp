...
struct DeviceAuthRequestBody {
  ...
  friend DeviceAuthRequestBody
  tag_invoke(const json::value_to_tag<DeviceAuthRequestBody> &,
             const json::value &jv) {
  ...
  friend void tag_invoke(const json::value_from_tag &, json::value &jv,
    ...
    if (b.expires_in)
      o["expires_in"] = *b.expires_in;
    if (b.device_code)
      o["device_code"] = *b.device_code;
    if (b.user_code)
  ...
  // Move-friendly helper for chaining: parse<T>().and_then(T::validated)
  static monad::MyResult<DeviceAuthRequestBody>
  validated(DeviceAuthRequestBody &&x) {
    auto r = x.validate();
    if (r.is_err()) {
      return monad::MyResult<DeviceAuthRequestBody>::Err(r.error());
    }
    return monad::MyResult<DeviceAuthRequestBody>::Ok(std::move(x));
  }
};
...
struct StartResp {
  ...
  // Move-friendly helper for chaining: parse<T>().and_then(T::validated)
  static monad::MyResult<StartResp> validated(StartResp &&x) {
    auto r = x.validate();
    if (r.is_err()) {
      return monad::MyResult<StartResp>::Err(r.error());
    }
    return monad::MyResult<StartResp>::Ok(std::move(x));
  }
...
struct PollResp {
  ...
  friend void tag_invoke(boost::json::value_from_tag, boost::json::value &jv,
    ...
    boost::json::object obj{{"status", r.status}};
    if (r.access_token && !r.access_token->empty()) {
      obj["access_token"] = *r.access_token;
    }
    if (r.refresh_token && !r.refresh_token->empty()) {
      obj["refresh_token"] = *r.refresh_token;
    }
    if (r.expires_in) {
      obj["expires_in"] = *r.expires_in;
    }
    if (r.registration_code && !r.registration_code->empty()) {
      obj["registration_code"] = *r.registration_code;
    }
  ...
  friend PollResp tag_invoke(const boost::json::value_to_tag<PollResp> &,
    ...
    if (auto *jo_p = jv.if_object()) {
      ...
      if (auto *p = jo_p->if_contains("refresh_token");
          p && p->is_string()) {
      ...
      if (auto *p = jo_p->if_contains("registration_code");
          p && p->is_string()) {
        r.registration_code = json::value_to<std::string>(*p);
      }
  ...
  monad::MyResult<void> validate() const {
    ...
    auto is_allowed_status = [](const std::string &s) {
      return s == "pending" || s == "authorization_pending" ||
             s == "slow_down" || s == "denied" || s == "access_denied" ||
  ...
  // Move-friendly helper for chaining: parse<T>().and_then(T::validated)
  static monad::MyResult<PollResp> validated(PollResp &&x) {
    auto r = x.validate();
    if (r.is_err()) {
      return monad::MyResult<PollResp>::Err(r.error());
    }
    return monad::MyResult<PollResp>::Ok(std::move(x));
  }
};
struct VerifyResp {
  std::string status;
  ...
  // Move-friendly helper for chaining: parse<T>().and_then(T::validated)
  static monad::MyResult<VerifyResp> validated(VerifyResp &&x) {
    auto r = x.validate();
    if (r.is_err()) {
      return monad::MyResult<VerifyResp>::Err(r.error());
    }
    return monad::MyResult<VerifyResp>::Ok(std::move(x));
  }
};
} // namespace deviceauth
