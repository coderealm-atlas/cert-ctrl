...
class ImportCaActionHandler
    : public std::enable_shared_from_this<ImportCaActionHandler> {
public:
  using Factory = std::function<std::shared_ptr<ImportCaActionHandler>()>;

  ImportCaActionHandler(certctrl::ICertctrlConfigProvider &config_provider,
  ...
  boost::asio::awaitable<monad::MyResult<void>>
  remove_ca(std::int64_t ca_id, std::optional<std::string> ca_name);
