...
VCPKG_TOOL_CACHE_NAME="vcpkg-${VCPKG_TOOL_RELEASE_TAG}-muslc"
...
if [[ ${#EFFECTIVE_PROXY_DEBUG[@]} -gt 0 ]]; then
  echo "[alpine-build] Effective proxy env (masked):" >&2
  printf '  %s\n' "${EFFECTIVE_PROXY_DEBUG[@]}" >&2
else
  echo "[alpine-build] Effective proxy env (masked): (none)" >&2
fi
